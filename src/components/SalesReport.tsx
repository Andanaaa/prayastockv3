...
export function SalesReport({ items, outgoingItems, onDeleteSale }: Props) {
  ...
  const getStockStatus = (stock: number, totalSales: number): StockStatus => {
    const remainingStock = stock - totalSales;
    if (remainingStock - totalSales * 2.5 > totalSales) return 'URGENT';
    if (remainingStock - totalSales * 1.75 > totalSales) return 'PREPARE';
    return 'SUFFICIENT';
  };

  const salesData = useMemo(() => {
    return items.map((item) => {
      let relevantSales = outgoingItems.filter((sale) => {
  ...
  }, [items, outgoingItems, period, selectedDate]);
        ...
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 md:space-x-4">
            ...
            <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
              ...
              <input
                type={period === 'MONTHLY' ? 'month' : 'week'}
                value={selectedDate}
